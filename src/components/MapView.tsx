...
export default function MapView({
  ...
  const addIslandLayers = useCallback((map: mapboxgl.Map) => {
    ...
    const showIslandPopup = (e: mapboxgl.MapMouseEvent & { features?: mapboxgl.MapGeoJSONFeature[] }) => {
      ...
      popup.setLngLat(coords).setHTML(`<div style="font-family:'Lora',serif;padding:4px 0;"><strong style="color:#6b4e0a;font-size:13px;">${props?.name || ""}</strong><br/><span style="color:#8b7a5a;font-size:11px;font-style:italic;">${props?.nameEn || ""}</span></div>`).addTo(map);
    };
